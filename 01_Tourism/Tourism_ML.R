...
# * Read Data ----
raw <- read_csv("00_Data/Sept_2022/tour_occ_nim__custom_3466548_linear.csv")
...
full_data_tbl <- data %>%
  ...
  group_by(geo) %>%
  future_frame(time, .length_out = 8, .bind_data = TRUE) %>%
  ungroup() %>%
  ...
  map(.f = function(df) {
    df %>%
      arrange(time) %>%
      #tk_augment_fourier(time, .periods = c(2, 6)) %>%
...
submodels_2_ids_to_keep <- c(4, 1, 7, 5)
...
fcs <- model_ensemble_refit_tbl %>%
  ...
  group_by(geo) %>%
  filter(.model_desc == "ENSEMBLE (MEDIAN): 4 MODELS")
